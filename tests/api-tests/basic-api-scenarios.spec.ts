...
/*  interface defines the specifications of an entity, therefore allows flexibility
    in such cases we expect certain datatype to return (while TS / JS wouldn't know what datatype to expect)   */
...
test.describe("API Tests Block", () => {
    ...
    test("Validate creating a new user by API call", async ({ request }) => {
    ...
    test("Validate user removal by API call", async ({ request }) => {
        
        const removedUserResponse = await request.delete(`${baseUrl}api/users/2`);

        expect(removedUserResponse.status()).toBe(204);
        expect(removedUserResponse.statusText()).toBe("No Content");
